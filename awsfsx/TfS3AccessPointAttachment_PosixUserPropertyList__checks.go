...
func (j *jsiiProxy_TfS3AccessPointAttachment_PosixUserPropertyList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*TfS3AccessPointAttachment_PosixUserProperty:
		val := val.(*[]*TfS3AccessPointAttachment_PosixUserProperty)
	...
	case []*TfS3AccessPointAttachment_PosixUserProperty:
		val_ := val.([]*TfS3AccessPointAttachment_PosixUserProperty)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *[]*TfS3AccessPointAttachment_PosixUserProperty; received %#v (a %T)", val, val)
		}
	}

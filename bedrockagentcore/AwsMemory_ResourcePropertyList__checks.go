...
func (j *jsiiProxy_AwsMemory_ResourcePropertyList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*AwsMemory_ResourceProperty:
		val := val.(*[]*AwsMemory_ResourceProperty)
	...
	case []*AwsMemory_ResourceProperty:
		val_ := val.([]*AwsMemory_ResourceProperty)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *[]*AwsMemory_ResourceProperty; received %#v (a %T)", val, val)
		}
	}

...
func (j *jsiiProxy_AwsDevicePool_RulePropertyList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*AwsDevicePool_RuleProperty:
		val := val.(*[]*AwsDevicePool_RuleProperty)
	...
	case []*AwsDevicePool_RuleProperty:
		val_ := val.([]*AwsDevicePool_RuleProperty)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *[]*AwsDevicePool_RuleProperty; received %#v (a %T)", val, val)
		}
	}

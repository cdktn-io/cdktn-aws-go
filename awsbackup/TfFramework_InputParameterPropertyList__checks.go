...
func (j *jsiiProxy_TfFramework_InputParameterPropertyList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*TfFramework_InputParameterProperty:
		val := val.(*[]*TfFramework_InputParameterProperty)
	...
	case []*TfFramework_InputParameterProperty:
		val_ := val.([]*TfFramework_InputParameterProperty)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *[]*TfFramework_InputParameterProperty; received %#v (a %T)", val, val)
		}
	}

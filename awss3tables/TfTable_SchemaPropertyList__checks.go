...
func (j *jsiiProxy_TfTable_SchemaPropertyList) validateSetInternalValueParameters(val interface{}) error {
	...
	case cdktn.IResolvable:
		// ok
	case *[]*TfTable_SchemaProperty:
		val := val.(*[]*TfTable_SchemaProperty)
	...
	case []*TfTable_SchemaProperty:
		val_ := val.([]*TfTable_SchemaProperty)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *[]*TfTable_SchemaProperty; received %#v (a %T)", val, val)
		}
	}

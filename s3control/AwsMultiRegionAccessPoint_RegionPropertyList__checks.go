...
func (j *jsiiProxy_AwsMultiRegionAccessPoint_RegionPropertyList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*AwsMultiRegionAccessPoint_RegionProperty:
		val := val.(*[]*AwsMultiRegionAccessPoint_RegionProperty)
	...
	case []*AwsMultiRegionAccessPoint_RegionProperty:
		val_ := val.([]*AwsMultiRegionAccessPoint_RegionProperty)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *[]*AwsMultiRegionAccessPoint_RegionProperty; received %#v (a %T)", val, val)
		}
	}

...
func (j *jsiiProxy_AwsTable_StaticColumnPropertyList) validateSetInternalValueParameters(val interface{}) error {
	...
	case cdktn.IResolvable:
		// ok
	case *[]*AwsTable_StaticColumnProperty:
		val := val.(*[]*AwsTable_StaticColumnProperty)
	...
	case []*AwsTable_StaticColumnProperty:
		val_ := val.([]*AwsTable_StaticColumnProperty)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *[]*AwsTable_StaticColumnProperty; received %#v (a %T)", val, val)
		}
	}

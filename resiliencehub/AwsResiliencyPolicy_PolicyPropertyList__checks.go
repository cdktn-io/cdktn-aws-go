...
func (j *jsiiProxy_AwsResiliencyPolicy_PolicyPropertyList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*AwsResiliencyPolicy_PolicyProperty:
		val := val.(*[]*AwsResiliencyPolicy_PolicyProperty)
	...
	case []*AwsResiliencyPolicy_PolicyProperty:
		val_ := val.([]*AwsResiliencyPolicy_PolicyProperty)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *[]*AwsResiliencyPolicy_PolicyProperty; received %#v (a %T)", val, val)
		}
	}

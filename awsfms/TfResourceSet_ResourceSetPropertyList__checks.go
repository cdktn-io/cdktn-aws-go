...
func (j *jsiiProxy_TfResourceSet_ResourceSetPropertyList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*TfResourceSet_ResourceSetProperty:
		val := val.(*[]*TfResourceSet_ResourceSetProperty)
	...
	case []*TfResourceSet_ResourceSetProperty:
		val_ := val.([]*TfResourceSet_ResourceSetProperty)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *[]*TfResourceSet_ResourceSetProperty; received %#v (a %T)", val, val)
		}
	}

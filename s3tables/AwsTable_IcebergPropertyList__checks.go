...
func (j *jsiiProxy_AwsTable_IcebergPropertyList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*AwsTable_IcebergProperty:
		val := val.(*[]*AwsTable_IcebergProperty)
	...
	case []*AwsTable_IcebergProperty:
		val_ := val.([]*AwsTable_IcebergProperty)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *[]*AwsTable_IcebergProperty; received %#v (a %T)", val, val)
		}
	}
